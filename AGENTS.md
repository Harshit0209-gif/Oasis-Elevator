...
- `OfficeMap.tsx` embeds the office location via the Google Maps Embed API, reading the key from an optional `VITE_GOOGLE_MAPS_API_KEY` env var; without one set, it falls back to an address card instead of a broken iframe. See the setup note below for obtaining a key.
